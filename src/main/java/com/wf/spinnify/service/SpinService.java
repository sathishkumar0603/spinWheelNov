...
import com.wf.spinnify.entity.WfRegionalForAmManagersEntity;
...
import com.wf.spinnify.repository.WfRegionalForAmManagersRepository;
...
public class SpinService {
	...
	private WfRegionalForAmManagersRepository forAmManagersRepository;
	...
	public SpinService(WfRegionalManagersRepository regionalManagersRepository,
			...
			SpinHelper helper, WfRegionalForAmManagersRepository forAmManagersRepository) {
		...
		this.helper = helper;
		this.forAmManagersRepository = forAmManagersRepository;
	}

	public String extractData(MultipartFile rm, MultipartFile am, MultipartFile store, MultipartFile rmForAm) {
		StringBuilder response = new StringBuilder();
		try (InputStream rmStream = rm.getInputStream();
				InputStream rmAmStream = rmForAm.getInputStream();
			...
			response.append("RM for Am File Data: ").append(processRmForAmFile(rmAmStream));
	...
	private Object processRmForAmFile(InputStream rmAmStream) {
		List<WfRegionalForAmManagersEntity> entities = new ArrayList<>();

		try (Workbook workbook = WorkbookFactory.create(rmAmStream)) {
			...
			for (Row row : sheet) {
				...
				WfRegionalForAmManagersEntity entity = new WfRegionalForAmManagersEntity();
			...
			forAmManagersRepository.saveAll(entities);

		} catch (Exception e) {
			e.printStackTrace();
			return "Error processing RM file: " + e.getMessage();
		}
		return "RM FOR AM data processed successfully.";
	}
	...
	public List<AmWinnersResponse> getAllAmWinners() {
		...
		try {
			List<WfRegionalForAmManagersEntity> entities = forAmManagersRepository.findAll();
			// Retrieve all previous winners and handle null case
			...
			for (WfRegionalForAmManagersEntity wfRegionalManagersEntity : entities) {
				...
				int winnerCount = (int) Math.round(amCount * 0.2); // Calculate 20%
